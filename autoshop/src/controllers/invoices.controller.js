...
async function generate(req, res) {
  ...
  try {
    ...
    // 5. Calculate totals
    const subtotal = parseFloat(servicesTotal.rows[0].total) + parseFloat(partsTotal.rows[0].total)
...
async function getOne(req, res) {
  const { id } = req.params
  try {
    // Invoice + customer + work order info
    const invoiceResult = await pool.query(
      `SELECT
         i.*,
         c.name    AS customer_name, c.phone AS customer_phone,
         c.rfc, c.business_name, c.fiscal_address,
    ...
    const services = await pool.query(
      `SELECT wos.hours, wos.price_at_time,
    ...
    const parts = await pool.query(
      `SELECT wop.quantity, wop.price_at_time,
